...
class MaterialMatchReportContractTests(unittest.TestCase):
    ...
    def test_validate_report_rejects_missing_declared_field(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            with self.assertRaisesRegex(RuntimeError, "generation_reasoning"):
                MODULE.validate_report(_report(generation_reasoning=""), Path(temp_dir) / "report.md")
