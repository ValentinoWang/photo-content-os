...
- Assumption IDs: none
- Invalidation keys: checklist.a1
- AC budget: 2
...
- Product Context refs: SRC-A1, source-notes.md
- Role Context refs: 本地内容创作者，以及可选配对的上游中台用户
- Resolved Surface Contract refs: .ssot/source-requirements.json#SURF-LOGIN
- Screen Contract ref: .ssot/source-requirements.json#SURF-LOGIN
- Visual Contract refs: .ssot/source-requirements.json
- UI Change declaration: agents-results/2026-09-02/openclaw-media-full-checklist-implementation/acceptance-fragments/OCM-A1/ui-change.json
- Human acceptance workspace: none

## User and scenario

本地内容创作者从受支持的真实入口使用“账号登录（邮箱 / Apple / 微信）”，必要时主动配对上游账号或本地第三方工具。
...
使用上游中台身份完成邮箱、Apple 或微信登录；配对始终可选，未登录、撤销或平台不支持时本地能力不退化。
...
When 用户查看或执行“账号登录（邮箱 / Apple / 微信）”
Then 系统完成“使用上游中台身份完成邮箱、Apple 或微信登录；配对始终可选，未登录、撤销或平台不支持时本地能力不退化。”，并显示真实进度、回执和下一步
```
...
| --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | machine/e2e | SRC-CHECKLIST-A1 | 使用上游中台身份完成邮箱、Apple 或微信登录；配对始终可选，未登录、撤销或平台不支持时本地能力不退化。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | Automatic | Yes |
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-A1 | 跳过、解除配对、会话过期及 Windows/Linux 不支持配对时，本地项目、素材和创作操作仍可完整使用。 | Automatic | Yes |
...
| --- | --- | --- | --- | --- |
| AC-01 | 正常闭环自动验收 | acceptance-fragments/OCM-A1/acceptance/machine/e2e/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-A1/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
