...
- Resolved Surface Contract refs: .ssot/surface-inventory.json#SURF-LOGIN
- Screen Contract ref: .ssot/interaction-matrix.json#SURF-LOGIN
...
| --- | --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | SRC-A1 | 使用上游中台身份完成邮箱、Apple 或微信登录；配对始终可选，未登录、撤销或平台不支持时本地能力不退化。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | E2E and local runtime | Automatic | Yes |
| AC-02 | behavior | SRC-A1 | 前置、权限、路径、版本、能力或外部系统异常时失败关闭，不伪造成功且可重试或恢复 | Integration, E2E and negative paths | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-A1/acceptance/machine/integration-contract/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
