...
- Assumption IDs: none
- Invalidation keys: checklist.t3
- AC budget: 2
...
本地内容创作者从受支持的真实入口使用“给 43_content_os_doctor.py 和 01_scan_media_manifest.py 补测试”，必要时主动配对上游账号或本地第三方工具。
...
给 43_content_os_doctor.py 和 01_scan_media_manifest.py 补测试：补基础用例。 01 至少要覆盖损坏文件、零时长、缺 EXIF 这几种边界。
...
When 用户查看或执行“给 43_content_os_doctor.py 和 01_scan_media_manifest.py 补测试”
Then 系统完成“给 43_content_os_doctor.py 和 01_scan_media_manifest.py 补测试：补基础用例。 01 至少要覆盖损坏文件、零时长、缺 EXIF 这几种边界。”，并显示真实进度、回执和下一步
```
...
| --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | machine/integration-contract | SRC-CHECKLIST-T3 | 给 43_content_os_doctor.py 和 01_scan_media_manifest.py 补测试：补基础用例。 01 至少要覆盖损坏文件、零时长、缺 EXIF 这几种边界。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | Automatic | Yes |
| AC-02 | behavior | machine/integration-contract | SRC-CHECKLIST-T3 | 媒体清单 schema 对路径、哈希、大小、设备与定位字段做双向校验；非法清单不得进入后续动作。 | Automatic | Yes |
...
| --- | --- | --- | --- | --- |
| AC-01 | 正常闭环自动验收 | acceptance-fragments/OCM-T3/acceptance/machine/integration-contract/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-T3/acceptance/machine/integration-contract/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
