...
| --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | machine/e2e | SRC-CHECKLIST-P3 | 结构化时间线为主视图，同时保留文本说明与受约束的选区修改，不形成第二份机器执行权威。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | Automatic | Yes |
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-P3 | 时间线与文本仅是同一 EDL 的两种视图；切换不得创建第二份权威，受约束选区修改必须带 revision。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-P3/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
