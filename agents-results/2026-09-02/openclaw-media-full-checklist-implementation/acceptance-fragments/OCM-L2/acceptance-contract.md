...
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-L2 | 分类、标签和用途筛选必须来自索引并可组合；未知条件返回空结果而不是放宽查询或静默显示全部。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-L2/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
