...
- Assumption IDs: none
- Invalidation keys: checklist.t4
- AC budget: 2
...
本地内容创作者从受支持的真实入口使用“给 analysis_tiering 的输出定 JSON Schema”，必要时主动配对上游账号或本地第三方工具。
...
给 analysis_tiering 的输出定 JSON Schema：补 schemas/analysis_tiering.schema.json，纳入现有的契约校验流程。
...
When 用户查看或执行“给 analysis_tiering 的输出定 JSON Schema”
Then 系统完成“给 analysis_tiering 的输出定 JSON Schema：补 schemas/analysis_tiering.schema.json，纳入现有的契约校验流程。”，并显示真实进度、回执和下一步
```
...
| --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | machine/integration-contract | SRC-CHECKLIST-T4 | 给 analysis_tiering 的输出定 JSON Schema：补 schemas/analysis_tiering.schema.json，纳入现有的契约校验流程。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | Automatic | Yes |
| AC-02 | behavior | machine/integration-contract | SRC-CHECKLIST-T4 | 分析策略版本、层级和预算由同一配置权威消费；耗尽时显式停止或降档，不得静默超额。 | Automatic | Yes |
...
| --- | --- | --- | --- | --- |
| AC-01 | 正常闭环自动验收 | acceptance-fragments/OCM-T4/acceptance/machine/integration-contract/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-T4/acceptance/machine/integration-contract/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
