...
- Assumption IDs: none
- Invalidation keys: checklist.t5
- AC budget: 2
...
本地内容创作者从受支持的真实入口使用“确认现有 Studio 的安全模型能不能撑住新界面”，必要时主动配对上游账号或本地第三方工具。
...
所有新增写接口沿用 loopback、同源、CSRF 和明确 revision 域的乐观并发控制，跨端口 Origin 必须拒绝。
...
When 用户查看或执行“确认现有 Studio 的安全模型能不能撑住新界面”
Then 系统完成“所有新增写接口沿用 loopback、同源、CSRF 和明确 revision 域的乐观并发控制，跨端口 Origin 必须拒绝。”，并显示真实进度、回执和下一步
```
...
| --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | machine/integration-contract | SRC-CHECKLIST-T5 | 所有新增写接口沿用 loopback、同源、CSRF 和明确 revision 域的乐观并发控制，跨端口 Origin 必须拒绝。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | Automatic | Yes |
| AC-02 | behavior | machine/integration-contract | SRC-CHECKLIST-T5 | 所有新增写接口强制 loopback、同源、CSRF 和真正整数 expectedRevision；bool、string、float 与跨端口 Origin 均拒绝。 | Automatic | Yes |
...
| --- | --- | --- | --- | --- |
| AC-01 | 正常闭环自动验收 | acceptance-fragments/OCM-T5/acceptance/machine/integration-contract/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-T5/acceptance/machine/integration-contract/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
