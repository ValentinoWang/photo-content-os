...
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-S1 | 预算五个字段可增减并持久化，均为非负整数；revision 冲突或非法值不得覆盖当前配置。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-S1/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
