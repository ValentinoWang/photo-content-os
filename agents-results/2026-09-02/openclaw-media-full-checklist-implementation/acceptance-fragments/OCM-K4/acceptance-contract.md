...
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-K4 | 研究参考与可剪素材分开存储和标识；外部参考不得自动进入媒体执行清单。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-K4/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
