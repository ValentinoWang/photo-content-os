...
- Assumption IDs: none
- Invalidation keys: checklist.d1
- AC budget: 2
...
- Product Context refs: SRC-D1, source-notes.md
...
- UI Change declaration: agents-results/2026-09-02/openclaw-media-full-checklist-implementation/acceptance-fragments/OCM-D1/ui-change.json
- Human acceptance workspace: none

## User and scenario

本地内容创作者从受支持的真实入口使用“整理台的「AI 自动分事件、分批」”，必要时主动配对上游账号或本地第三方工具。
...
从媒体清单生成可解释、可确认且不可拆分实况照片组的事件批次计划；未确认或输入漂移时禁止迁移。
...
When 用户查看或执行“整理台的「AI 自动分事件、分批」”
Then 系统完成“从媒体清单生成可解释、可确认且不可拆分实况照片组的事件批次计划；未确认或输入漂移时禁止迁移。”，并显示真实进度、回执和下一步
```
...
| --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | machine/e2e | SRC-CHECKLIST-D1 | 从媒体清单生成可解释、可确认且不可拆分实况照片组的事件批次计划；未确认或输入漂移时禁止迁移。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | Automatic | Yes |
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-D1 | 分批建议在用户确认前不得移动文件；确认必须携带 planDigest、批次、目标项目和 expectedRevision，计划漂移返回冲突。 | Automatic | Yes |
...
| --- | --- | --- | --- | --- |
| AC-01 | 正常闭环自动验收 | acceptance-fragments/OCM-D1/acceptance/machine/e2e/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-D1/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
