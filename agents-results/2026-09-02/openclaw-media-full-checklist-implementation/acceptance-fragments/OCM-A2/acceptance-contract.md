...
- Resolved Surface Contract refs: .ssot/source-requirements.json#SURF-SETUP
- Screen Contract ref: .ssot/source-requirements.json#SURF-SETUP
...
| --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | machine/e2e | SRC-CHECKLIST-A2 | 向导可重入地完成位置、运行环境、编辑器、账号与设备四步，并能从失败处恢复。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | Automatic | Yes |
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-A2 | 四步向导逐步 CAS 保存；中断后从最后成功步骤恢复，账号步骤可跳过且不得把本地能力改为只读。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-A2/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
