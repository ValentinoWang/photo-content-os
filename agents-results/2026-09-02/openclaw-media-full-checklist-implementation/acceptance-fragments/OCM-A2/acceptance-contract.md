...
- Decision refs: decision.scope.full-checklist@1, decision.archive.location-lifecycle@1, decision.identity.optional-upstream@1, decision.jianying.historical-only@1
- Assumption IDs: none
- Invalidation keys: checklist.a2
- AC budget: 2
...
- Product Context refs: SRC-A2, source-notes.md
- Role Context refs: 本地内容创作者，以及可选配对的上游中台用户
- Resolved Surface Contract refs: .ssot/surface-inventory.json#SURF-SETUP
- Screen Contract ref: .ssot/interaction-matrix.json#SURF-SETUP
- Visual Contract refs: .ssot/visual-fidelity-contract.json
- UI Change declaration: agents-results/2026-09-02/openclaw-media-full-checklist-implementation/acceptance-fragments/OCM-A2/ui-change.json
- Human acceptance workspace: none

## User and scenario

本地内容创作者从受支持的真实入口使用“安装向导的四步（存放位置 / 运行环境 / 剪辑器 / 账号与设备）”，必要时主动配对上游账号或本地第三方工具。
...
向导可重入地完成位置、运行环境、编辑器、账号与设备四步，并能从失败处恢复。
...
When 用户查看或执行“安装向导的四步（存放位置 / 运行环境 / 剪辑器 / 账号与设备）”
Then 系统完成“向导可重入地完成位置、运行环境、编辑器、账号与设备四步，并能从失败处恢复。”，并显示真实进度、回执和下一步
```
...
| --- | --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | SRC-A2 | 向导可重入地完成位置、运行环境、编辑器、账号与设备四步，并能从失败处恢复。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | E2E and local runtime | Automatic | Yes |
| AC-02 | behavior | SRC-A2 | 前置、权限、路径、版本、能力或外部系统异常时失败关闭，不伪造成功且可重试或恢复 | Integration, E2E and negative paths | Automatic | Yes |
...
| --- | --- | --- | --- | --- |
| AC-01 | 正常闭环自动验收 | acceptance-fragments/OCM-A2/acceptance/machine/e2e/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-A2/acceptance/machine/integration-contract/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
