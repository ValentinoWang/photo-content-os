...
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-S2 | 生命周期和物理位置分别持久化并回读；受控路径越界、重复身份或不可访问位置返回可操作错误。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-S2/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
