...
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-D2 | 生产删除只能生成建议；未勾选、未二次确认、回收站不可用或回读失败时不得移动任何媒体。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-D2/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
