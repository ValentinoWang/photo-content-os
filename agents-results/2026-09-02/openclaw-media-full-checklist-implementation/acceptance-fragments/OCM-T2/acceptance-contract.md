...
| --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | machine/integration-contract | SRC-CHECKLIST-T2 | 给素材库三件套补测试（12 / 14 / 15）：和第 00 节的索引层一起做，定契约的同时补测试。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | Automatic | Yes |
| AC-02 | behavior | machine/integration-contract | SRC-CHECKLIST-T2 | 重复照片选择保留实况绑定和用户保留规则；阈值边界、空组及无候选都有确定结果。 | Automatic | Yes |
...
| --- | --- | --- | --- | --- |
| AC-01 | 正常闭环自动验收 | acceptance-fragments/OCM-T2/acceptance/machine/integration-contract/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
