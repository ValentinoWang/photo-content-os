...
- Assumption IDs: none
- Invalidation keys: checklist.t2
- AC budget: 2
...
本地内容创作者从受支持的真实入口使用“给素材库三件套补测试（12 / 14 / 15）”，必要时主动配对上游账号或本地第三方工具。
...
给素材库三件套补测试（12 / 14 / 15）：和第 00 节的索引层一起做，定契约的同时补测试。
...
When 用户查看或执行“给素材库三件套补测试（12 / 14 / 15）”
Then 系统完成“给素材库三件套补测试（12 / 14 / 15）：和第 00 节的索引层一起做，定契约的同时补测试。”，并显示真实进度、回执和下一步
```
...
| --- | --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | SRC-T2 | 给素材库三件套补测试（12 / 14 / 15）：和第 00 节的索引层一起做，定契约的同时补测试。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | E2E and local runtime | Automatic | Yes |
| AC-02 | behavior | SRC-T2 | 前置、权限、路径、版本、能力或外部系统异常时失败关闭，不伪造成功且可重试或恢复 | Integration, E2E and negative paths | Automatic | Yes |
...
| --- | --- | --- | --- | --- |
| AC-01 | 正常闭环自动验收 | acceptance-fragments/OCM-T2/acceptance/machine/e2e/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-T2/acceptance/machine/integration-contract/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
