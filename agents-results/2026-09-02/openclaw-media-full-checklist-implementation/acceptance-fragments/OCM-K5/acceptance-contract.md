...
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-K5 | 复盘与发布记录绑定候选摘要和 revision；失败发布不得写成成功，重试保留前次证据。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-K5/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
