...
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-S3 | 诊断项由运行时动态生成，不写死六项或固定数量；复制报告必须等于当前无密钥诊断 JSON。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-S3/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
