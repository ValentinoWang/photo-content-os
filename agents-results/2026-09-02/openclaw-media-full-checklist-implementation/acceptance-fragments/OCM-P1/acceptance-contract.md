...
| --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | machine/e2e | SRC-CHECKLIST-P1 | 剪辑决策条目列表（时间码 / 台词 / 角色标签）：纯前端。EDL 已经通过 GET /api/projects/:id 返回了。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | Automatic | Yes |
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-P1 | 项目读取必须返回同一 revision 的文档和 EDL 投影；缺失或无效 EDL 显示阻断，不得从界面文本拼出权威。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-P1/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
