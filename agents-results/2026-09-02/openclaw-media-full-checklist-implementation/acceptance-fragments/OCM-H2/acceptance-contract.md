...
- Screen Contract ref: .ssot/source-requirements.json#SURF-DASHBOARD
...
| --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | machine/e2e | SRC-CHECKLIST-H2 | 聚合数据中台、Codex、ChatCut Desktop 本地 MCP 与本机引擎的实时状态；单项失败不拖垮整体。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | Automatic | Yes |
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-H2 | 能力状态来自实时健康、上游与 ChatCut 探测；超时、未配对和不支持必须分开显示，不得写死可用。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-H2/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
