...
- Assumption IDs: none
- Invalidation keys: checklist.k2
- AC budget: 2
...
- Product Context refs: SRC-K2, source-notes.md
...
- UI Change declaration: agents-results/2026-09-02/openclaw-media-full-checklist-implementation/acceptance-fragments/OCM-K2/ui-change.json
- Human acceptance workspace: none

## User and scenario

本地内容创作者从受支持的真实入口使用“版本 diff 与非破坏性回滚”，必要时主动配对上游账号或本地第三方工具。
...
版本 diff 与非破坏性回滚：原型里没有版本概念。至少要在项目屏留一个入口。
...
When 用户查看或执行“版本 diff 与非破坏性回滚”
Then 系统完成“版本 diff 与非破坏性回滚：原型里没有版本概念。至少要在项目屏留一个入口。”，并显示真实进度、回执和下一步
```
...
| --- | --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | SRC-K2 | 版本 diff 与非破坏性回滚：原型里没有版本概念。至少要在项目屏留一个入口。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | E2E and local runtime | Automatic | Yes |
| AC-02 | behavior | SRC-K2 | 前置、权限、路径、版本、能力或外部系统异常时失败关闭，不伪造成功且可重试或恢复 | Integration, E2E and negative paths | Automatic | Yes |
...
| --- | --- | --- | --- | --- |
| AC-01 | 正常闭环自动验收 | acceptance-fragments/OCM-K2/acceptance/machine/e2e/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-K2/acceptance/machine/integration-contract/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
