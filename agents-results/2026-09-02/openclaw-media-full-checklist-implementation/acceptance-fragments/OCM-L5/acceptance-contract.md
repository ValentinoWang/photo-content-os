...
| --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | machine/e2e | SRC-CHECKLIST-L5 | 详情栏主按钮「选择项目并加入」要落到真实动作：要么补 16 号能力，要么这个按钮先降级为「复制卡片路径」这类真能做到的动作。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | Automatic | Yes |
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-L5 | 导入、登记新资产、加入项目和在访达中显示分别有真实动作；未知 assetId、路径越界或 revision 冲突时拒绝。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-L5/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
