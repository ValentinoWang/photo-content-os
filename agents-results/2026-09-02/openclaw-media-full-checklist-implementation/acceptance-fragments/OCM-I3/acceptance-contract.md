...
| --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | machine/e2e | SRC-CHECKLIST-I3 | 连拍识别（「发现 4 组连拍」）与实况配对：把 12 的输出定契约（JSON + schema）、补测试，再接进批次分析流程。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | Automatic | Yes |
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-I3 | 归档预览逐项显示来源、目标、数量与碰撞；预览不得创建目录或移动媒体，确认后回执必须可重放核对。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-I3/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
