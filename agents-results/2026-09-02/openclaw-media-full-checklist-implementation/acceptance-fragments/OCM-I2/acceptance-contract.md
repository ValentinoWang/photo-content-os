...
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-I2 | 分批依据必须展示时间、位置和实况照片不可拆分原因；缺少元数据时使用明确兜底且不拆散绑定组。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-I2/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
