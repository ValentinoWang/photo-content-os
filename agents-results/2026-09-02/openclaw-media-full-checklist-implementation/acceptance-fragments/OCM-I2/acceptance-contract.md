...
- Decision refs: decision.scope.full-checklist@1, decision.organizer.auto-batching@1
- Assumption IDs: none
- Invalidation keys: checklist.i2
- AC budget: 2
...
- Product Context refs: SRC-I2, source-notes.md
...
- UI Change declaration: agents-results/2026-09-02/openclaw-media-full-checklist-implementation/acceptance-fragments/OCM-I2/ui-change.json
- Human acceptance workspace: none

## User and scenario

本地内容创作者从受支持的真实入口使用“批次卡上的来源构成（iPhone ×6 · 屏幕录制 ×2 · 相机 ×1）”，必要时主动配对上游账号或本地第三方工具。
...
批次卡上的来源构成（iPhone ×6 · 屏幕录制 ×2 · 相机 ×1）：读 manifest 分组计数即可，不用新后端。
...
When 用户查看或执行“批次卡上的来源构成（iPhone ×6 · 屏幕录制 ×2 · 相机 ×1）”
Then 系统完成“批次卡上的来源构成（iPhone ×6 · 屏幕录制 ×2 · 相机 ×1）：读 manifest 分组计数即可，不用新后端。”，并显示真实进度、回执和下一步
```
...
| --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | machine/e2e | SRC-CHECKLIST-I2 | 批次卡上的来源构成（iPhone ×6 · 屏幕录制 ×2 · 相机 ×1）：读 manifest 分组计数即可，不用新后端。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | Automatic | Yes |
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-I2 | 前置、权限、路径、版本、能力或外部系统异常时失败关闭，不伪造成功且可重试或恢复 | Automatic | Yes |
...
| --- | --- | --- | --- | --- |
| AC-01 | 正常闭环自动验收 | acceptance-fragments/OCM-I2/acceptance/machine/e2e/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-I2/acceptance/machine/integration-contract/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
