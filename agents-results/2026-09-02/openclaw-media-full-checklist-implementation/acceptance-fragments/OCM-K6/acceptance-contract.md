...
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-K6 | Brief 与脚本保持独立版本阶段，脚本只从获批 Brief 和素材匹配输入编译；反向编辑不得改写 Brief。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-K6/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
