...
| AC-02 | behavior | machine/local-runtime | SRC-CHECKLIST-L3 | 界面同时展示生命周期与每个物理位置的独立清单、校验值和回读状态；登记位置不得冒充副本存在。 | Automatic | Yes |
...
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-L3/acceptance/machine/local-runtime/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
