...
- Assumption IDs: none
- Invalidation keys: checklist.t1
- AC budget: 2
...
本地内容创作者从受支持的真实入口使用“给 35_promote_inbox_batch_to_project.py 补测试”，必要时主动配对上游账号或本地第三方工具。
...
给 35_promote_inbox_batch_to_project.py 补测试：接 UI 之前先补测试。这条建议优先级高于任何界面工作。
...
When 用户查看或执行“给 35_promote_inbox_batch_to_project.py 补测试”
Then 系统完成“给 35_promote_inbox_batch_to_project.py 补测试：接 UI 之前先补测试。这条建议优先级高于任何界面工作。”，并显示真实进度、回执和下一步
```
...
| --- | --- | --- | --- | --- | --- |
| AC-01 | behavior | machine/integration-contract | SRC-CHECKLIST-T1 | 给 35_promote_inbox_batch_to_project.py 补测试：接 UI 之前先补测试。这条建议优先级高于任何界面工作。；从真实入口完成正常业务闭环，回读结果与可见状态一致 | Automatic | Yes |
| AC-02 | behavior | machine/integration-contract | SRC-CHECKLIST-T1 | 破坏性批次迁移必须有同 planDigest 的预览、幂等回执和可恢复日志；重复调用不得重复移动。 | Automatic | Yes |
...
| --- | --- | --- | --- | --- |
| AC-01 | 正常闭环自动验收 | acceptance-fragments/OCM-T1/acceptance/machine/integration-contract/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
| AC-02 | 失败、重试与恢复自动验收 | acceptance-fragments/OCM-T1/acceptance/machine/integration-contract/runs/&lt;run-id&gt;/result.md | Automatic | Yes |
